...
echo -e "\e[33m# Green Deployment on \e[1m$APP_HOSTNAME ...\e[0m"
  ...
  then
    ...
    echo -e "\e[33m# Sending Green Artifact to Server\e[0m"
...
ssh -o StrictHostKeyChecking=no "$APP_USERNAME@$APP_HOSTNAME" "
    ...
    if [ -f "${APP_HOME_PATH}/artifacts-$CI_COMMIT_REF_SLUG-$CI_COMMIT_SHORT_SHA.tar.gz" ]; then
      echo -e '\e[33m# Extract Green Release\e[0m'
    ...
    echo -e '\e[33m# Activating Green Realese\e[0m'
    ln -sf ${APP_LATEST_RELEASE_PATH}/$APP_USERNAME ${APP_GREEN_PATH}

    echo -e '\e[33m# Restarting $APP_NAME (Green Release)\e[0m'
    sudo systemctl restart $APP_NAME-green.service
  "
...
#  APP_HOSTNAME=$(printf "${APP_NAME}-%02d.app.afra.snapp.infra" ${i})
  ...
  ssh -o StrictHostKeyChecking=no "$APP_USERNAME@$APP_HOSTNAME" "
    ...
    echo -e '\e[33m# Stopping Green Release ...\e[0m'
    sudo systemctl stop $APP_NAME-green.service
  "
  echo
#done
